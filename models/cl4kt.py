...
class CL4KT(Module):
    ...
    def get_attention_maps(self):
        """Get attention maps from the model's transformer layers.
        
        Returns:
            dict: Dictionary containing attention maps from:
                - First and last question encoder layers
                - First and last interaction encoder layers
                - First and last knowledge retriever layers
        """
        attention_maps = {
            'question_encoder': {
                'first_layer': {},
        ...
        # Get attention maps from question encoder
        if len(self.question_encoder) > 0:
            first_layer = self.question_encoder[0].masked_attn_head
            last_layer = self.question_encoder[-1].masked_attn_head
            
            if hasattr(first_layer, 'attention_maps'):
                attn_maps = first_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['question_encoder']['first_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
            
            if hasattr(last_layer, 'attention_maps'):
                attn_maps = last_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['question_encoder']['last_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
            
        # Get attention maps from interaction encoder
        if len(self.interaction_encoder) > 0:
            first_layer = self.interaction_encoder[0].masked_attn_head
            last_layer = self.interaction_encoder[-1].masked_attn_head
            
            if hasattr(first_layer, 'attention_maps'):
                attn_maps = first_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['interaction_encoder']['first_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
            ...
            if hasattr(last_layer, 'attention_maps'):
                attn_maps = last_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['interaction_encoder']['last_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
            
        # Get attention maps from knowledge retriever
        if len(self.knoweldge_retriever) > 0:
            first_layer = self.knoweldge_retriever[0].masked_attn_head
            last_layer = self.knoweldge_retriever[-1].masked_attn_head
            
            if hasattr(first_layer, 'attention_maps'):
                attn_maps = first_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['knowledge_retriever']['first_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
            ...
            if hasattr(last_layer, 'attention_maps'):
                attn_maps = last_layer.attention_maps
                avg_attn_maps = attn_maps[0, :, :].detach().cpu().numpy()
                attention_maps['knowledge_retriever']['last_layer'] = {
                    'attn_maps': avg_attn_maps,  # All heads are shared
                    'n_shared_heads': avg_attn_maps.shape[0]  # All heads are shared
                }
